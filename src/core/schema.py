"""
Core Data Schemas & Validation Contracts
========================================
Architecture Role:
    Defines the canonical Pydantic data models, status types, and validation boundaries
    governing chemical Safety Data Sheet (SDS) ingestion, retrieval, reflection,
    and final verdict generation across the platform.

Core Data Contracts:
    1. ValidStatus:
       Strict three-state classification taxonomy:
       - 'EXACT MATCH': Substance, manufacturer, and jurisdiction verified with high confidence (>= 50%).
       - 'BEST AVAILABLE': Substance and manufacturer matched; minor variance in jurisdiction, date,
         or distributor vs direct host; confidence >= 30%.
       - 'NEEDS REVIEW': Unverified, conflicting identity, or ungrounded candidate requiring human review;
         mandates empty final_url and confidence = 0%.
    2. NormalizedSDSRequest:
       Represents the four-pillar chemical search identity extracted from user spreadsheets:
       Product Name, Manufacturer / Company, Language, and Country / Jurisdiction.
    3. SDSEvidence:
       Structured evidence extracted from candidate documents (PDF/HTML), encapsulating
       GHS 16-section text chunks, CAS numbers, catalog part numbers, revision dates, and authenticity.
    4. ActionDecision:
       Discrete action selection schema generated by the agent's decision engine
       (SEARCH, RANK, FETCH, VERIFY, FINISH, RETRY).
    5. VerificationResult:
       Structured output from the independent reflection / verification gate, assessing
       Product Match, Manufacturer Match, Part/CAS Match, and Jurisdiction/Language Match.
    6. SDSValidationResult:
       Strict final output schema enforcing confidence bounds, non-empty grounded URLs
       for positive verdicts, and provenance traceability.
"""
...
# Strict three-state SDS classification taxonomy
ValidStatus = Literal["EXACT MATCH", "BEST AVAILABLE", "NEEDS REVIEW"]

# Discrete actions supported by the LangGraph agent state machine
