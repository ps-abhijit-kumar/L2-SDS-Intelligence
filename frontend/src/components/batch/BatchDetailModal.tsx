import React from 'react';
import { X, ExternalLink, ShieldCheck } from 'lucide-react';
import { StatusBadge } from '../common/StatusBadge';
import { ConfidenceGauge } from '../common/ConfidenceGauge';
import { Button } from '../common/Button';
import { BatchRow } from '../../types/sds';

interface BatchDetailModalProps {
  row: BatchRow | null;
  onClose: () => void;
}

export const BatchDetailModal: React.FC<BatchDetailModalProps> = ({ row, onClose }) => {
  if (!row) return null;

  const isResolved = row.Status && row.Status !== 'PENDING';

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6 bg-slate-950/85 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-3xl max-h-[90vh] bg-[#0B1020] border border-white/[0.1] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-slide-up">
        {/* Modal Header */}
        <div className="px-6 py-4 border-b border-white/[0.08] flex items-center justify-between bg-[#070A12] shrink-0">
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              <h3 className="text-base font-bold text-slate-100">
                {row['Product Name'] || row.Product}
              </h3>
              <StatusBadge status={row.Status || 'PENDING'} size="sm" />
            </div>
            <p className="text-xs text-slate-400 font-mono">
              {row._sheet_name ? `Worksheet: ${row._sheet_name} • ` : ''}
              Row: #{row._excel_row || row._row_index + 1} (S.No. {row['S.No.']})
            </p>
          </div>

          <button
            onClick={onClose}
            className="p-1.5 text-slate-400 hover:text-slate-100 hover:bg-white/[0.08] rounded-lg transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Modal Content */}
        <div className="p-6 space-y-5 overflow-y-auto flex-1 text-xs">
          {/* Target Attributes Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 bg-[#070A12] p-4 rounded-xl border border-white/[0.06]">
            <div>
              <span className="text-slate-500 font-mono font-bold uppercase text-[10px]">Product</span>
              <p className="font-semibold text-slate-200 mt-0.5 truncate">{row.Product || 'N/A'}</p>
            </div>
            <div>
              <span className="text-slate-500 font-mono font-bold uppercase text-[10px]">Manufacturer</span>
              <p className="font-semibold text-slate-200 mt-0.5 truncate">{row['Product Company Name'] || 'Unspecified'}</p>
            </div>
            <div>
              <span className="text-slate-500 font-mono font-bold uppercase text-[10px]">Part No / ID</span>
              <p className="font-semibold text-slate-200 mt-0.5 truncate">{row['Part Number'] || '—'}</p>
            </div>
            <div>
              <span className="text-slate-500 font-mono font-bold uppercase text-[10px]">Jurisdiction</span>
              <p className="font-semibold text-slate-200 mt-0.5">{row.Country || 'Global'}</p>
            </div>
            <div>
              <span className="text-slate-500 font-mono font-bold uppercase text-[10px]">Language</span>
              <p className="font-semibold text-slate-200 mt-0.5">{row.Language || 'English'}</p>
            </div>
          </div>

          {/* Confidence Score */}
          {isResolved && row.Confidence !== undefined && (
            <div className="p-4 bg-[#070A12] rounded-xl border border-white/[0.06]">
              <ConfidenceGauge confidence={row.Confidence} size="md" />
            </div>
          )}

          {/* Verified Document URL */}
          {row['Found URL'] ? (
            <div className="space-y-1.5">
              <span className="text-cyan-400 font-mono font-bold uppercase text-[10px] tracking-wider">
                Confirmed Safety Data Sheet URL
              </span>
              <div className="flex items-center justify-between gap-3 bg-[#070A12] p-3.5 rounded-xl border border-cyan-500/30 shadow-glow-cyan">
                <span className="font-mono text-slate-200 truncate select-all">
                  {row['Found URL']}
                </span>
                <a
                  href={row['Found URL']}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="shrink-0 inline-flex items-center gap-1.5 text-cyan-400 hover:text-cyan-300 font-semibold bg-cyan-950/80 px-3 py-1.5 rounded-lg border border-cyan-500/40"
                >
                  <span>Open SDS</span>
                  <ExternalLink className="w-3.5 h-3.5" />
                </a>
              </div>
            </div>
          ) : (
            <div className="p-3 bg-[#070A12] rounded-xl border border-white/[0.06] text-slate-400">
              {row.Status === 'PENDING'
                ? 'This row is waiting to be processed by the batch agent.'
                : 'No validated SDS document URL was confirmed by the agent.'}
            </div>
          )}

          {/* Agent Reasoning & Compliance Rationale */}
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-slate-200 font-semibold">
              <ShieldCheck className="w-4 h-4 text-cyan-400" />
              <span>Compliance Rationale & Agent Validation</span>
            </div>
            <div className="p-4 rounded-xl bg-[#070A12] border border-white/[0.08] leading-relaxed text-slate-300 whitespace-pre-wrap font-sans">
              {row.Reasoning || 'No reasoning recorded yet for this row.'}
            </div>
          </div>
        </div>

        {/* Footer */}
        <div className="px-6 py-3.5 border-t border-white/[0.08] bg-[#070A12] flex justify-end shrink-0">
          <Button variant="secondary" size="sm" onClick={onClose}>
            Close
          </Button>
        </div>
      </div>
    </div>
  );
};
